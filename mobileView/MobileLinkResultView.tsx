            ...
            <div className="space-y-4 mobile-fade-in">
              <div className="mobile-editorial-card rounded-xl p-4 border-l-4 border-l-slate-900">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-2 py-0.5 rounded-sm border text-[10px] font-black uppercase tracking-tighter serif italic ${
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                    <p className="text-[9px] font-black text-emerald-700 uppercase tracking-widest">Фактическа Точност</p>
                    <p className="text-lg font-black text-emerald-900">{(stats.factualAccuracy * 100).toFixed(0)}%</p>
                  </div>
                  <div className="rounded-lg bg-blue-50 border border-blue-200 p-3">
                    <p className="text-[9px] font-black text-blue-700 uppercase tracking-widest">Логическа Стройност</p>
                    <p className="text-lg font-black text-blue-900">{(stats.logicalSoundness * 100).toFixed(0)}%</p>
                  </div>
                  <div className="rounded-lg bg-orange-50 border border-orange-200 p-3">
                    <p className="text-[9px] font-black text-orange-700 uppercase tracking-widest">Емоционална Окраска</p>
                    <p className="text-lg font-black text-orange-900">{(stats.emotionalBias * 100).toFixed(0)}%</p>
                  </div>
                  <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                    <p className="text-[9px] font-black text-red-700 uppercase tracking-widest">Пропаганден Индекс</p>
                    <p className="text-lg font-black text-red-900">{(stats.propagandaScore * 100).toFixed(0)}%</p>
                  </div>
                </div>
                ...
                {analysis.summary.recommendations && (
                  <div className="mt-4 pt-4 border-t border-slate-200">
                    <h3 className="text-[9px] font-black text-amber-900 uppercase tracking-widest mb-2">Стратегически Препоръки</h3>
                    <p className="text-xs text-slate-700 leading-relaxed whitespace-pre-wrap">{analysis.summary.recommendations}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4 mobile-fade-in">
              <div className="mobile-editorial-card rounded-xl p-4 border-t-2 border-t-slate-900">
                <div className="text-center mb-6 pb-4 border-b-2 border-slate-900">
                  ...
                  <h2 className="text-2xl font-black text-slate-900 italic serif">Заключителен Анализ</h2>
                </div>
                <div className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap serif">
                  {analysis.summary?.finalInvestigativeReport || analysis.summary?.overallSummary || 'Докладът се генерира.'}
                </div>
              </div>
            </div>
          )}
