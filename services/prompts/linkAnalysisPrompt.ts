export const getLinkAnalysisPrompt = (url: string, scrapedContent?: string, hasImages = false): string => {
...
URL НА СТАТИЯТА: ${url}
${hasImages ? `
СНИМКИ: По-долу са приложени снимки от статията. Анализирай ги задълбочено.
` : ''}
${hasContent ? `
...
` : ''}
КРИТИЧНО ВАЖНО:
1. **ОСНОВЕН ИСТОЧНИК**: ${hasContent ? 'Ползвай ТЕКСТА НА СТАТИЯТА по-горе като основен источник. Анализирай САМО тази статия — не измисляй, не заменяй с друга.' : 'Търси в Google по URL и заглавие за пълния текст на статията. Текстът е основният източник за анализ.'}
2. **GOOGLE SEARCH — САМО ЗА ВЕРИФИКАЦИЯ**: Използвай Google Search единствено за: верификация на фактически твърдения, контекст за автора и медията, намиране на коментари и алтернативни медии. НЕ го използвай за намиране на текста на статията.
3. **ПРОЧЕТИ ЦЯЛАТА СТАТИЯ**: Прочети ВСЕКИ параграф, ВСЯКО твърдение, ВСЯКО число.
4. **МАКСИМАЛЕН ДЕТАЙЛ**: Целта е ИЗЧЕРПАТЕЛЕН анализ. Извлечи ВСЯКО твърдение и ВСЯКА манипулация.
5. **НАМЕРИ КОМЕНТАРИТЕ**: Търси с Google Search коментари към тази статия (Disqus, Facebook, Reddit, Twitter/X). Ако намериш — анализирай. Ако няма — commentsAnalysis: null.
6. **СРАВНИ С ДРУГИ МЕДИИ**: Намери как 2-3 РАЗЛИЧНИ медии отразяват СЪЩАТА тема.
7. **ВРЪЩАНЕТО НА МАЛКО ДАННИ Е КРИТИЧНА ГРЕШКА** — бъди изключително изчерпателен!
...
${hasImages ? `
7. ВИЗУАЛЕН АНАЛИЗ (ЗАДЪЛЖИТЕЛНО — приложени са снимки): За всяка снимка напиши:
- Описание: какво показва снимката и как се свързва с конкретна част от текста
- Съответствие с текста: дали снимката подкрепя, неутрална е или противоречи на написаното
- Разминавания и манипулация: има ли несъответствие между визуала и текста (misleading image, изваждане извън контекст, емоционално зареждане чрез образи, cherry-picked visuals)
- Визуални манипулации: misleading graphics, подправени снимки, подбрани кадри, които изкривяват посланието на текста
ВАЖНО: visualAnalysis трябва да е ДЕТАЙЛЕН текст (минимум 3–5 изречения). НЕ оставяй празно.
` : ''}
